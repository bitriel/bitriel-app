...
export async function approveForHandler(amount) {
  ...
  const approveAmount = BigInt(amount * Math.pow(10, 12))
...
export async function selendraToWrap(amount) {
  ...
  const signer = await Signer()
  // eslint-disable-next-line no-undef
  const swapAmount = BigInt(amount * Math.pow(10, 12))
...
export async function wrapTransfer(substrateAdress, amount) {
  ...
  // eslint-disable-next-line no-undef
  const transferAmount = BigInt(amount * Math.pow(10, 12))
  ...
  const data =
    '0x' +
    ethers.utils
      .hexZeroPad(ethers.BigNumber.from(transferAmount).toHexString(), 32)
